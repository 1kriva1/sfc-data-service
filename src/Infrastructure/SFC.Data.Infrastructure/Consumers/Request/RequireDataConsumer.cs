...
namespace SFC.Data.Infrastructure.Consumers.Request;
    ...
    : IConsumer<RequireData>
{
    ...
    {
        GetRequestDataModel model = await _dataService.GetRequestDataAsync().ConfigureAwait(true);

        InitializeData command = _mapper.BuildRequestInitializeDataCommand(model);
...
public class RequireDataConsumerDefinition : ConsumerDefinition<RequireDataConsumer>
{
    ...
    private Message Exchange { get { return _settings.Exchanges.Request.Value.Data.RequireInitialize; } }

    public RequireDataConsumerDefinition(IConfiguration configuration)
    {
        _settings = configuration.GetRabbitMqSettings();
        EndpointName = "sfc.data.request.initialize.require.queue";
    }
